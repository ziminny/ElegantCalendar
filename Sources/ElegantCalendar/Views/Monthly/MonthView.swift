...
private extension MonthView {
    ...
    var monthText: some View {
        Text(month.fullMonth.uppercased())
            ...
            .tracking(7)
            .foregroundColor(
                isWithinSameMonthAndYearAsToday ? .orange :
                (colorScheme == .dark ? .white : .black)
                //isWithinSameMonthAndYearAsToday ? theme.primary : .primary
            )
    }
    ...
    var yearText: some View {
        Text(month.year)
            ...
            .foregroundColor(
                isWithinSameMonthAndYearAsToday ? .orange :
                (colorScheme == .dark ? .white : .black)
               // isWithinSameMonthAndYearAsToday ? theme.primary : .gray
            )
            .opacity(0.95)
    }
...
private struct CalendarAccessoryView: View, @preconcurrency MonthlyCalendarManagerDirectAccess {
    ...
    private var daysFromTodayText: some View {
        ...
        let daysDescription = isBeforeToday ? "DIAS ATRÁS" : "DIAS A PARTIR DE HOJE"
